...
Claude Code 执行任务时，有时需要确认权限、补充上下文或等待你的输入，通常只能阻塞等待。Agent Hotline 解决了这个问题：它通过飞书给你发消息，你用手机回复，Claude 就能继续工作。

```
Claude Code ──Hook──> Agent Hotline ──API──> 飞书（话题 + 按钮）
             <──注入──              <──WS── （点击 / 回复）
```
...
> **macOS 辅助功能权限**：系统设置 > 隐私与安全性 > 辅助功能 > 添加 Terminal.app。此权限是终端注入的前提。

搞定。照常使用 `claude` 即可 —— 无需 tmux，无需包装器，无需特殊配置。

## 工作原理

1. Claude Code [Hooks](https://docs.anthropic.com/en/docs/claude-code/hooks) 在任务完成 / 需要权限 / 等待输入时触发
2. `agent-hotline hook` 将事件 POST 到本地服务器
3. Agent Hotline 在飞书中创建**话题消息**（项目名作为标题，内容作为首条回复）
4. 你点击按钮或回复文字 —— 通过飞书 WebSocket 实时送达
5. 你的回复被注入到对应的 Terminal.app 标签页（通过 AppleScript）
...
送达状态以表情回应的形式展示在你的消息上，不会在话题中产生额外噪音。
...
```
终端标签页 1（project-a）  <-->  飞书话题「project-a | 重构代码...」
终端标签页 2（project-b）  <-->  飞书话题「project-b | 新增功能...」
```
...
agent-hotline test-inject /dev/ttys003 "hi"  # 测试终端注入
```
